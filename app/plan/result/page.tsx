  ...
  return <CustomerShell step={4}>
    ...
    <section className="multi-result-layout">
      <div className="multi-result-main">{results.map(({ functionPlan, estimate }, index) => <article className="result-card function-result-card" key={functionPlan.id}>
        ...
        <details className="function-menu-details function-team-details"><summary>View recommended service team <span>{estimate.manpowerItems.reduce((sum, item) => sum + item.quantity, 0)} staff</span></summary>
          <div className="public-team-grid">{estimate.manpowerItems.map((item) => <div key={`${item.kind}-${item.role}`}><span>{item.kind === 'kitchen' ? 'Kitchen' : 'Service'}</span><b>{item.role}</b><strong>× {item.quantity}</strong></div>)}</div>
        </details>
        <div className="price-summary compact-summary"><div><span>Menu</span><b>{money(estimate.menuPricePerPlate)} / person</b></div><div><span>Service team</span><b>{money(estimate.servicePricePerPlate)} / person</b></div><div><span>Event services</span><b>{money(estimate.operationsPricePerPlate)} / person</b></div><div className="total"><span>Function estimate</span><b>{money(estimate.estimatedEventTotal)}</b></div></div>
      </article>)}</div>
